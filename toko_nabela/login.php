...
if (isset($_POST['login'])) {
    ...
    $q = mysqli_query($koneksi, "SELECT * FROM users WHERE email = '$email_esc' LIMIT 1");
    ...
    if ($user && password_verify($password, $user['password'])) {
        ...
        if ($user['role'] === 'admin') {
            ...
            $tujuan = $redirect && $redirect !== 'login.php' ? $redirect : 'index.php';
...
<head>
    ...
    <title>Masuk | Kedai Nabela</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        .auth-card { max-width: 400px; width: 100%; background: var(--card-bg); border: 1px solid var(--border-color); border-radius: 14px; padding: 32px; }
        ...
        input { width: 100%; padding: 10px 12px; border: 1px solid var(--border-color); border-radius: 8px; font-size: 13.5px; background: var(--item-bg); color: var(--text-main); font-family: inherit; }
...
<div class="auth-card">
    ...
    <div class="footer-text">Belum punya akun? <a href="register.php">Daftar sebagai customer</a></div>
